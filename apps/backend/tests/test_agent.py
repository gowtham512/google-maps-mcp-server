...
async def test_run_agent_loop_with_tool_call(monkeypatch):
    ...
    # New design: exactly two messages per turn — user + final assistant.
    assert len(result["messages"]) == 2
    assert result["messages"][0]["role"] == "user"
    assistant = result["messages"][1]
    assert assistant["role"] == "assistant"
    # Enriched tool_calls carry name/input/result/status on the assistant message.
    tool_calls = assistant["tool_calls"]
    assert len(tool_calls) == 1
    assert tool_calls[0]["name"] == "search_places"
    assert tool_calls[0]["status"] == "done"
    assert "Hotel X" in tool_calls[0]["result"]
    assert tool_calls[0]["input"]["text_query"] == "hotels in Paris"
